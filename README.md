...
If you want a simple workflow to follow without forgetting anything, use this:

1. Train the hard agent with the hard-specific PPO preset.

```bash
.venv/bin/python src/train.py --env hard --total-timesteps 1000000 --no-graphics --train-worker-id 30 --eval-worker-id 31 --run-name hard_tuned_v2
```

2. Visually inspect the best saved model.

```bash
.venv/bin/python src/play.py --env hard --model runs/hard/hard_tuned_v2/best_model.zip --time-scale 1.0 --worker-id 33
```

3. Save a final quantitative summary for the report.

```bash
.venv/bin/python src/evaluate.py --env hard --model runs/hard/hard_tuned_v2/best_model.zip --episodes 20 --no-graphics --worker-id 32 --out runs/hard/hard_tuned_v2/eval_once.csv
```

4. Plot the evaluation curve for the report.

```bash
.venv/bin/python src/plot_metrics.py --csv runs/hard/hard_tuned_v2/evaluations.csv --out runs/hard/hard_tuned_v2/eval_plot.png
```

Files to keep for the report:

- `runs/hard/hard_tuned_v2/train_config.json`
- `runs/hard/hard_tuned_v2/evaluations.csv`
- `runs/hard/hard_tuned_v2/eval_once.csv`
- `runs/hard/hard_tuned_v2/eval_plot.png`
- `runs/hard/hard_tuned_v2/best_model.zip`

If the hard agent is still weak, try this next ablation:

```bash
.venv/bin/python src/train.py --env hard --total-timesteps 1000000 --no-graphics --train-worker-id 40 --eval-worker-id 41 --run-name hard_ablation_lr5e5 --learning-rate 5e-5
```
